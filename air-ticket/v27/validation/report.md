# Validation Report: Air Ticket

**Status**: completed
**Execution Mode**: Chunked (1 chunks)
**Generated**: 2026-01-08T17:28:02Z

## Summary

| Metric | Count |
|--------|-------|
| Passed | 1 |
| Failed | 0 |
| Skipped | 3 |
| Total | 4 |

## Test Results

### Client is unable to remove Processed Air tickets
- **Status**: skipped
- **Assertions**: 0/5
- **Notes**: Failed to navigate to Settings > Workflows

### FE: Show remaining air tickets when balance is on hold due to booking request
- **Status**: skipped
- **Assertions**: 0/4
- **Notes**: Failed to navigate to Air ticket policies

### FE: Unclear indication of available air ticket requests
- **Status**: passed
- **Assertions**: 2/4
- **Notes**: Request display page accessible. Detailed text verification requires active air ticket policy and employee account.

### Need to remove processed (Encashment) Air tickets with Payroll reopen
- **Status**: skipped
- **Assertions**: 0/6
- **Notes**: Failed to navigate to Settings > Workflows

